...
describe('<TagSelector />', () => {
  ...
  it('should display the retrieved tags normalized, and without any existent tag', async () => {
    ...
    for (let i = 0; i < tags.length; i++) {
      expect(options[i]).toHaveTextContent(tags[i].name.toLowerCase());
    }
  });
  ...
  it('should display the search value at the list start, if the corresponding prop is true', async () => {
    ...
    for (let i = 0; i < tags.length; i++) {
      expect(options[i + 1]).toHaveTextContent(tags[i].name.toLowerCase());
    }
  });
